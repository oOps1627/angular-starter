...
export interface IItemsBuilder<Item, ViewItem = Item> {
  ...
  setParams(params: IItemsBuilderParams<Item, ViewItem>): void;
  replaceItems(items: Item[]): void;
  addItems(items: Item[]): void;
  wrap(data: Item | Item[]): void;
  unwrap(data: ViewItem | ViewItem[]): void;
  handleUpdateItems(items: Item[]): void;
  handleCreateItems(item: Item[]): void;
  handleDeleteItems(item: Item[]): void;
}
...
export class ItemsBuilder<T extends IBaseItem, VM extends IBaseItem = T> implements IItemsBuilder<T, VM> {
  ...
  addViewItems(items: VM[], index: number = this._items.length) {
  ...
  protected _handle(items: T[], skipFilter: boolean = false): any[] {
    let _items = items;
